...
for line in open("SABE_1172_UNHESMSV_genomemapping/02_NRS_GENMAP.tsv", "r"):
    ...
    if chrom == "chr1" and int(start) >= 2592000 and int(end) <= 2845000:
        nrs_list.add(nrs)
        nrs_loc[nrs] = loc
        nrs_bp[nrs] = bp
...
####
outfile = open("00_NRS_TTC34_inf.tsv", "w")
outfile.write("NRS\tLength\tGenFreq\tGENMAP\tNovelPeptides\tExprInCohorts\tFreqInCohorts\tSick/TotalPatients\tOddsRatio\tDGE\n")

outfile_bed = open("00_TTC43_NRS.bed", "w")
nrs_keep = set()
...
####
fasta_out = open("00_NRS_TTC34.fasta", "w")
